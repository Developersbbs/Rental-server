...
exports.getAllBills = async (req, res) => {
  try {
    let { page = 1, limit = 10, startDate, endDate, paymentStatus, type } = req.query;
    ...
    if (type === 'rental') {
      // Robust filter: Check for type 'rental' OR existence of rentalDetails.rentalId
      filter.$or = [
        { type: 'rental' },
        { 'rentalDetails.rentalId': { $exists: true, $ne: null } }
      ];
    } else if (type) {
      filter.type = type;
    }
...
exports.createBill = async (req, res) => {
  try {
    ...
    if (customer.isBlocked) {
      throw httpError(403, 'This customer is blocked and cannot create new bills.');
    }
...
exports.downloadInvoicePDF = async (req, res) => {
  try {
    ...
    // Fetch bill with populated customer
    const bill = await Bill.findById(req.params.id).populate('customerId');
    if (!bill) {
      return res.status(404).json({ message: 'Bill not found' });
    }

    // For rental bills, fetch the rental details
    let rental = null;
    if (bill.type === 'rental' && bill.rentalDetails?.rentalId) {
      rental = await Rental.findById(bill.rentalDetails.rentalId)
        .populate('customer')
        .populate('items.item');
    }

    // Generate PDF
    const pdfBuffer = await generateInvoicePDF(bill, rental);
    ...
    res.status(500).json({ message: 'Failed to generate PDF invoice', error: err.message });
  }
};
...
exports.recordPayment = async (req, res) => {
  try {
    const { id } = req.params;
    const { amount, paymentMethod, paymentAccountId, paymentDate, notes } = req.body;
    ...
    if (paymentAmount > bill.dueAmount) {
      return res.status(400).json({
        message: `Payment amount ₹${paymentAmount} exceeds due amount ₹${bill.dueAmount}`
      });
    }
    ...
    if (paymentAccountId) {
      ...
      if (paymentAccount.status !== 'active') {
        return res.status(400).json({ message: 'Payment account is inactive' });
      }
    }

    // Calculate new amounts
    const newPaidAmount = roundToTwo(bill.paidAmount + paymentAmount);
    const newDueAmount = roundToTwo(bill.totalAmount - newPaidAmount);
    ...
    // Update the main payment method to the latest one used
    bill.paymentMethod = normalizePaymentMethod(paymentMethod);

    await bill.save();

    // Update payment account balance
    if (paymentAccount) {
      paymentAccount.currentBalance = roundToTwo(paymentAccount.currentBalance + paymentAmount);
      await paymentAccount.save();
    }

    // Update customer outstanding balance
    const previousDueAmount = roundToTwo(bill.dueAmount + paymentAmount);
    const outstandingDelta = roundToTwo(newDueAmount - previousDueAmount);

    if (bill.customerId && bill.customerId._id) {
      await Customer.findByIdAndUpdate(
        bill.customerId._id,
    ...
    await bill.populate('customerId', 'name email phone');
    await bill.populate('paymentHistory.paymentAccount', 'name accountType');
